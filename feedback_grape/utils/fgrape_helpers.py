...
def convert_system_params(system_params):
    ...
    def _Gate_validity_checks(gate):
        ...
        """
        if (
            not gate.measurement_flag
        ):  # Check if gate is callable on initial parameters and unitary
            unitary = gate.gate(gate.initial_params)

            assert (
                len(unitary.shape) == 2
                and unitary.shape[0] == unitary.shape[1]
            ), "The provided gate is not a square matrix."

            if not jnp.allclose(
                unitary @ unitary.conj().T, jnp.eye(unitary.shape[0])
            ):
                if jnp.allclose(unitary, unitary.conj().T):
                    raise ValueError(
                        "The provided gate is not unitary but Hermitian. Did you perhaps provide a Hamiltonian instead of a unitary?"
                    )
                else:
                    raise ValueError(
                        "The provided gate is not unitary. Did you perhaps mistake jnp.exp for a matrix exponential jax.scipy.linalg.expm?"
                    )
            ...
            for M in [M_0, M_1]:
                assert M.shape[0] == M.shape[1], (
                    "The provided measurement operator is not a square matrix."
                )

                E = M.conj().T @ M
                assert is_positive_semi_definite(E), (
                    "The provided measurement operator M does not satisfy M^† M >= 0 (positive semi-definite)."
                )

            assert jnp.allclose(
                M_0.conj().T @ M_0 + M_1.conj().T @ M_1, jnp.eye(M_0.shape[0])
            ), "The provided measurement operators do not sum to the identity."
